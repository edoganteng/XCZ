                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zXCZ ought to be enough for anybody." - Bill Gates, 2017
    ui->zXCZpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzXCZSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zXCZpayAmount->setFocus();
    }
}
/* disable MINT
 *
void PrivacyDialog::on_pushButtonMintzXCZ_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXCZ is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zXCZ, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zXCZ...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zXCZ in ") +
...
void PrivacyDialog::on_pushButtonSpendzXCZ_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXCZ is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zXCZ, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zXCZ
        sendzXCZ();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zXCZ
    sendzXCZ();
}

void PrivacyDialog::on_pushButtonZXczControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZXczControlDialog* zXczControl = new ZXczControlDialog(this);
    zXczControl->setModel(walletModel);
    zXczControl->exec();
}

void PrivacyDialog::setZXczControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzXczSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzXCZ()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid XChainZ Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zXCZpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zXCZ is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " XCZ </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zXCZ</b>";
    ...
    // use mints from zXCZ selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZXczControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZXczControlDialog::GetSelectedMints();
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zXCZ transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zXCZ was spent successfully update the addressbook with the label
    ...
    // Clear zxcz selector in case it was used
    ZXczControlDialog::setSelectedMints.clear();
    ui->labelzXczSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zXCZ Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Xcz, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.IsZerocoinMint())
            strStats += tr("zXCZ Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zxczTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zXCZ </b>";
    ...
    //ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zXCZ "));
    //ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zXCZ "));
    //ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zXCZ "));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zXCZ </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zXCZ </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zXCZ </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzXCZSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    //bool fButtonsEnabled =  ui->pushButtonMintzXCZ->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zXCZ
        //ui->pushButtonMintzXCZ->setEnabled(false);
        //ui->pushButtonMintzXCZ->setToolTip(tr("zXCZ is currently disabled due to maintenance."));

        // Spend zXCZ
        ui->pushButtonSpendzXCZ->setEnabled(false);
        ui->pushButtonSpendzXCZ->setToolTip(tr("zXCZ is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zXCZ
        //ui->pushButtonMintzXCZ->setEnabled(true);
        //ui->pushButtonMintzXCZ->setToolTip(tr("PrivacyDialog", "Enter an amount of XCZ to convert to zXCZ", 0));

        // Spend zXCZ
        ui->pushButtonSpendzXCZ->setEnabled(true);
        ui->pushButtonSpendzXCZ->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
