...
#include "ui_zxczcontroldialog.h"
...
std::set<std::string> ZXczControlDialog::setSelectedMints;
std::set<CMintMeta> ZXczControlDialog::setMints;

bool CZXczControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZXczControlDialog::COLUMN_DENOMINATION || column == ZXczControlDialog::COLUMN_VERSION || column == ZXczControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZXczControlDialog::ZXczControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZXczControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZXczControlDialog::~ZXczControlDialog()
{
    delete ui;
}

void ZXczControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZXczControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZXczControlWidgetItem* itemDenom(new CZXczControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZXczControlWidgetItem *itemMint = new CZXczControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zxczTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zxczTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zXCZ.";
            else if (!mint.isSeedCorrect)
                strReason = "The zXCZ seed used to mint this zXCZ is not the same as currently hold in the wallet";
            else
...
void ZXczControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZXczControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZXcz_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZXczControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZXczControlDialog::GetSelectedMints()
{
...
void ZXczControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
