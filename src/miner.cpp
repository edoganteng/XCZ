...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

	{
		...
		for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
			...
			for (const CTxIn& txin : tx.vin) {
				...
				if (hasZerocoinSpends) {
					...
					//Priority = (age^6+100000)*amount - gives higher priority to zxczs that have been in mempool long
					//and higher priority to zxczs that are large in value
					...
					// zXCZ spends can have very large priority, use non-overflowing safe functions
		...
		while (!vecPriority.empty()) {
			...
			// double check that there are no double spent zXCZ spends in this block or tx
			if (tx.HasZerocoinSpendInputs()) {
				...
				//This zXCZ serial has already been included in the block, do not add this tx.
				if (fDoubleSerial)
					continue;
			}
		...
		if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
			//For the period before v2 activation, zXCZ will be disabled and previous block's checkpoint is all that will be needed
		...
		if (fProofOfStake) {
			...
			if (pblock->IsZerocoinStake()) {
				...
				if (!pwallet->GetZerocoinKey(bnSerial, key)) {
					LogPrintf("%s: failed to find zXCZ with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
					return NULL;
				}

				//Sign block with the zXCZ key
				if (!SignBlockWithKey(*pblock, key)) {
					LogPrintf("BitcoinMiner(): Signing new block with zXCZ key failed \n");
					return NULL;
				}
			}
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
	...
	// Found a solution
	{
		...
		if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
			return error("XChainZMiner : generated block is stale");
	}
	...
	if (!ProcessNewBlock(state, NULL, pblock)) {
		if (pblock->IsZerocoinStake()) {
			pwalletMain->zxczTracker->RemovePending(pblock->vtx[1].GetHash());
			pwalletMain->zxczTracker->ListMints(true, true, true); //update the state
		}
		return error("XChainZMiner : ProcessNewBlock, block not accepted");
	}
...
{
	LogPrintf("XChainZMiner started\n");
	SetThreadPriority(THREAD_PRIORITY_LOWEST);
	RenameThread("xchainz-miner");
	...
	while (fGenerateBitcoins || fProofOfStake) {
		...
		LogPrintf("Running XChainZMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
