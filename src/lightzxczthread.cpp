...
#include "lightzxczthread.h"
#include "main.h"

/****** Thread ********/
void CLightWorker::ThreadLightZXCZSimplified() {
    RenameThread("xchainz-light-thread");
    ...
    while (true) {
        try {
            ...
            LogPrintf("%s pop work for %s \n\n", "xchainz-light-thread", genWit.toString());
            ...
            if (!pIndex) {
                ...
                if (blockHeight >= Params().Zerocoin_Block_V2_Start()) {
                    ...
                    try {
                        ...
                        LogPrintStr(std::string("ThreadLightZXCZSimplified: ") + e.message + "\n");
                    ...
                    if (!res) {
                        ...
                        ss << heightStop;
                        if (genWit.getPfrom()) {
                            LogPrintf("%s pushing message to %s \n", "xchainz-light-thread", genWit.getPfrom()->addrName);
            ...
            PrintExceptionContinue(&e, "lightzxczthread");
            break;
        }
    }
...
void CLightWorker::rejectWork(CGenWit& wit, int blockHeight, uint32_t errorNumber) {
    if (wit.getStartingHeight() == blockHeight){
        LogPrintf("%s rejecting work %s , error code: %s\n", "xchainz-light-thread", wit.toString(), errorNumber);
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
