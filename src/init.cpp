...
void Shutdown()
{
	...
	if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
		lightWorker.StopLightZxczThread();
	}
#ifdef ENABLE_WALLET
	delete pwalletMain;
	pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
	...
	strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "xchainz.conf"));
...
#ifndef WIN32
	strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "xchainzd.pid"));
#endif
	strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
	strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
	strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the XCZ and zXCZ money supply statistics") + " " + _("on startup"));
	...
	strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 14815, 51474));
	...
	if (GetBoolArg("-help-debug", false))
		strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in XCZ/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
			FormatMoney(CWallet::minTxFee.GetFeePerK())));
	strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in XCZ/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
	...
	string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, xchainz, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero, precompute, staking)"; // Don't translate these and qt below
	if (mode == HMM_BITCOIN_QT)
		debugCategories += ", qt";
	...
	strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in XCZ/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
	...
	strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all XChainZ specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
	strUsage += HelpMessageGroup(_("Staking options:"));
	strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
	strUsage += HelpMessageOpt("-xczstake=<n>", strprintf(_("Enable or disable staking functionality for XCZ inputs (0-1, default: %u)"), 1));
	strUsage += HelpMessageOpt("-zxczstake=<n>", strprintf(_("Enable or disable staking functionality for zXCZ inputs (0-1, default: %u)"), 1));
	...
	strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:14815"));
	...
	strUsage += HelpMessageOpt("-backupzxcz=<n>", strprintf(_("Enable automatic wallet backups triggered after each zXCZ minting (0-1, default: %u)"), 1));
	strUsage += HelpMessageOpt("-precompute=<n>", strprintf(_("Enable precomputation of zXCZ spends and stakes (0-1, default %u)"), 1));
	...
	strUsage += HelpMessageOpt("-zxczbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zXCZ backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
	...
	strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 14814, 51475));
...
std::string LicenseInfo()
{
	return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
		...
		"\n" +
		FormatParagraph(strprintf(_("Copyright (C) %i The XChainZ Developers"), COPYRIGHT_YEAR)) + "\n" +
		"\n" +
...
/** Sanity checks
 *  Ensure that XChainZ is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
	...
	// Sanity check
	if (!InitSanityCheck())
		return InitError(_("Initialization sanity check failed. XChainZ is shutting down."));
...
#endif
	// Make sure only a single XChainZ process is using the data directory.
	...
	if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
		return InitError(strprintf(_("Cannot obtain a lock on data directory %s. XChainZ is probably already running."), strDataDir));
	...
	while (!fLoaded) {
		...
		do {
			try {
				...
				//XChainZ specific: zerocoin and spork DB's
				...
				// XChainZ: load previous sessions sporks if we have them.
				...
				if (GetBoolArg("-reindexmoneysupply", false)) {
					RecalculateZXCZMinted();
					RecalculateZXCZSpent();
					RecalculateXCZSupply(1);
				}
				...
				if (GetBoolArg("-reindexaccumulators", false)) {
					...
					// XChainZ: recalculate Accumulator Checkpoints that failed to database properly
	...
	if (fDisableWallet) {
		...
		if (nLoadWalletRet != DB_LOAD_OK) {
			if (nLoadWalletRet == DB_CORRUPT)
				...
				strErrors << _("Error loading wallet.dat: Wallet requires newer version of XChainZ") << "\n";
			else if (nLoadWalletRet == DB_NEED_REWRITE) {
				strErrors << _("Wallet needed to be rewritten: restart XChainZ to complete") << "\n";
		...
		zwalletMain = new CzXCZWallet(pwalletMain->strWalletFile);
		...
		//Inititalize zXCZWallet
		uiInterface.InitMessage(_("Syncing zXCZ wallet..."));

		pwalletMain->InitAutoConvertAddresses();

		bool fEnableZXczBackups = GetBoolArg("-backupzxcz", true);
		pwalletMain->setZXczAutoBackups(fEnableZXczBackups);

		//Load zerocoin mint hashes to memory
		pwalletMain->zxczTracker->Init();
	...
	/* Denominations
	   ...
	   For example:
	   1XCZ+1000 == (.1XCZ+100)*10
	   10XCZ+10000 == (1XCZ+1000)*10
	*/
	...
	if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
		...
		lightWorker.StartLightZxczThread(threadGroup);
	}
	...
	if (pwalletMain) {
		...
		if (GetBoolArg("-precompute", false)) {
			// Run a thread to precompute any zXCZ spends
