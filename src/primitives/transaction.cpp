...
{
    // The wallet only allows publicSpend inputs in the same tx and not a combination between xcz and zxcz
    for(const CTxIn& txin : vin) {
...
CAmount CTransaction::GetValueOut() const
{
    ...
    {
        // XChainZ: previously MoneyRange() was called here. This has been replaced with negative check and boundary wrap check.
        if (it->nValue < 0)
