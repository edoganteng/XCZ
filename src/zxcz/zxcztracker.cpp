...
CzXCZTracker::CzXCZTracker(std::string strWalletFile)
{
...
CzXCZTracker::~CzXCZTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzXCZTracker::Init()
{
...
bool CzXCZTracker::Archive(CMintMeta& meta)
{
...
bool CzXCZTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzXCZTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzXCZTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzXCZTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzXCZTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzXCZTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzXCZTracker::GetSerialHashes()
{
...
CAmount CzXCZTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzXCZTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzXCZTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzXCZTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzXCZTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzXCZTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzXCZTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzXCZTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzXCZTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzXCZTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzXCZTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzXCZWallet* zXCZWallet)
{
    bool iszXCZWalletInitialized = (NULL != zXCZWallet);
    CMintMeta meta;
    ...
    if (! iszXCZWalletInitialized)
        zXCZWallet = new CzXCZWallet(strWalletFile);
    meta.isSeedCorrect = zXCZWallet->CheckSeed(dMint);
    if (! iszXCZWalletInitialized)
        delete zXCZWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzXCZTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzXCZTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzXCZTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzXCZTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzXCZTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzXCZTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzXCZWallet* zXCZWallet = new CzXCZWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zXCZWallet);
        }
        delete zXCZWallet;
        LogPrint("zero", "%s: added %d dzxcz from DB\n", __func__, listDeterministicDB.size());
    }
