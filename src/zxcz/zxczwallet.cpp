...
CzXCZWallet::CzXCZWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zxcz seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZXCZSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZXCZSeed_deprecated()) {
                    LogPrintf("%s: Updated zXCZ seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zxcz seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zxcz wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzXCZWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZXCZCount(nCountLastUsed);
    else if (!walletdb.ReadZXCZCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzXCZWallet::Lock()
{
    seedMaster = 0;
}

void CzXCZWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzXCZWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZXCZ(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzXCZWallet::LoadMintPoolFromDB()
{
...
void CzXCZWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzXCZWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzXCZWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zxczTracker->HasPubcoinHash(pMint.first)) {
...
bool CzXCZWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZXCZ(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zxczTracker which also adds to database
    pwalletMain->zxczTracker->Add(dMint, true);
...
void CzXCZWallet::SeedToZXCZ(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzXCZWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzXCZWallet::GenerateDeterministicZXCZ(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzXCZWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzXCZWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzXCZWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
